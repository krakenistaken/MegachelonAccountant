  ...
  try {
    ...
    const attendancesRs = await db.execute({
      sql: `SELECT 
            ...
            FROM attendances
            WHERE employee_id = ? AND status IN ('Geldi', 'Yarım Gün', 'Mesai') AND (paid_amount < daily_wage OR is_paid = 0)
            ORDER BY date ASC`,
      args: [empId],
    });
    ...
    for (const att of unpaidDays) {
      ...
      if (targetAccountId && paymentForDay > 0) {
        const statusLabel =
          att.status === 'Yarım Gün'
            ? ' - Yarım Gün'
            : att.status === 'Mesai'
            ? ' - Mesai (1.5x)'
            : '';
        const txDesc = `Maaş / Borç Kapatma (${att.date} günü${statusLabel}): ${employee.first_name} ${employee.last_name}${note ? ` - ${note}` : ''}`;
